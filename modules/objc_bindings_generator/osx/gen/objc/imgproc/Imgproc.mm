...
//
+ (void)GaussianBlur:(Mat*)src dst:(Mat*)dst ksize:(Size2i*)ksize sigmaX:(double)sigmaX sigmaY:(double)sigmaY borderType:(BorderTypes)borderType hint:(AlgorithmHint)hint {
    cv::GaussianBlur(src.nativeRef, dst.nativeRef, ksize.nativeRef, sigmaX, sigmaY, (cv::BorderTypes)borderType, (cv::AlgorithmHint)hint);
}
...
//
+ (void)HoughLines:(Mat*)image lines:(Mat*)lines rho:(double)rho theta:(double)theta threshold:(int)threshold srn:(double)srn stn:(double)stn min_theta:(double)min_theta max_theta:(double)max_theta use_edgeval:(BOOL)use_edgeval {
    cv::HoughLines(image.nativeRef, lines.nativeRef, rho, theta, threshold, srn, stn, min_theta, max_theta, (bool)use_edgeval);
}
...
//
+ (void)cvtColor:(Mat*)src dst:(Mat*)dst code:(ColorConversionCodes)code dstCn:(int)dstCn hint:(AlgorithmHint)hint {
    cv::cvtColor(src.nativeRef, dst.nativeRef, (cv::ColorConversionCodes)code, dstCn, (cv::AlgorithmHint)hint);
}
...
//
+ (void)cvtColorTwoPlane:(Mat*)src1 src2:(Mat*)src2 dst:(Mat*)dst code:(int)code hint:(AlgorithmHint)hint {
    cv::cvtColorTwoPlane(src1.nativeRef, src2.nativeRef, dst.nativeRef, code, (cv::AlgorithmHint)hint);
}
...
//
+ (void)approxPolyN:(Mat*)curve approxCurve:(Mat*)approxCurve nsides:(int)nsides epsilon_percentage:(float)epsilon_percentage ensure_convex:(BOOL)ensure_convex {
    cv::approxPolyN(curve.nativeRef, approxCurve.nativeRef, nsides, epsilon_percentage, (bool)ensure_convex);
}

+ (void)approxPolyN:(Mat*)curve approxCurve:(Mat*)approxCurve nsides:(int)nsides epsilon_percentage:(float)epsilon_percentage {
    cv::approxPolyN(curve.nativeRef, approxCurve.nativeRef, nsides, epsilon_percentage);
}

+ (void)approxPolyN:(Mat*)curve approxCurve:(Mat*)approxCurve nsides:(int)nsides {
    cv::approxPolyN(curve.nativeRef, approxCurve.nativeRef, nsides);
}
